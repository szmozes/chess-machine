...
public class Queen extends Piece {
	...
	public void paint(Graphics g, int size) {
		...
		case WHITE:
			BufferedImage img1;
			try {
				img1 = ImageIO.read(new File("white_queen.png"));
				g.drawImage(img1, field.column*size, field.row*size, size, size, null);
			} catch (IOException e) {
				System.out.println("white_queen.png didn't found");
			}
			break;
		case BLACK: 
			BufferedImage img2;
			try {
				img2 = ImageIO.read(new File("black_queen.png"));
				g.drawImage(img2, field.column*size, field.row*size, size, size, null);
			} catch (IOException e) {
				System.out.println("black_queen.png didn't found");
			}
			break;
		}
	}
