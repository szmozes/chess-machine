...
public class Bishop extends Piece {
	...
	public void paint(Graphics g, int size) {
		...
		case WHITE:
			BufferedImage img1;
			try {
				img1 = ImageIO.read(new File("white_bishop.png"));
				g.drawImage(img1, field.column*size, field.row*size, size, size, null);
			} catch (IOException e) {
				System.out.println("white_bishop.png didn't found");
			}
			break;
		case BLACK: 
			BufferedImage img2;
			try {
				...
				img2 = ImageIO.read(new File(imagePath, "black_bishop.png"));
				g.drawImage(img2, field.column*size, field.row*size, size, size, null);
			} catch (IOException e) {
				System.out.println("black_bishop.png didn't found");
			}
			break;
		}
	}
